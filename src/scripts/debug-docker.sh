...
echo "=========================================="
echo "🔍 Docker Debug Report - Haal VPS"
echo "=========================================="
echo ""
...
if docker exec Haal-postgres pg_isready -U postgres > /dev/null 2>&1; then
    echo -e "${GREEN}✅ OK${NC}"
else
    echo -e "${RED}❌ FAILED${NC}"
fi
...
echo -n "Redis: "
if docker exec Haal-redis redis-cli ping > /dev/null 2>&1; then
    echo -e "${GREEN}✅ OK${NC}"
else
    echo -e "${RED}❌ FAILED${NC}"
fi
...
EXITED=$(docker ps -a --filter "status=exited" --format "{{.Names}}" | grep -E "Haal|buildflow" || true)
if [ ! -z "$EXITED" ]; then
