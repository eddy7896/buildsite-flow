# 🚀 Haal Production Deployment Guide

Complete guide for deploying Haal to production using Docker.
...
docker exec Haal-postgres pg_dump -U postgres buildflow_db > database/backups/backup_$(date +%Y%m%d_%H%M%S).sql
```
...
# Verify nginx config
docker exec Haal-frontend nginx -t
```
