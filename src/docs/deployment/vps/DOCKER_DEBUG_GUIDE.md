...
docker exec Haal-postgres psql -U postgres -d buildflow_db -c "SELECT version();"
...
# Check database logs
docker logs Haal-postgres

# Test connection
docker exec Haal-postgres psql -U postgres -d buildflow_db -c "\dt"

# Check database is ready
docker exec Haal-postgres pg_isready -U postgres
```
...
echo -n "Database: "
docker exec Haal-postgres pg_isready -U postgres 2>/dev/null && echo "OK" || echo "FAILED"
echo ""
