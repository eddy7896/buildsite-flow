...
export const useGST = () => {
  ...
  const fetchSettings = async () => {
    if (!user || !profile?.agency_id) {
      console.log('Cannot fetch GST settings: user not authenticated or no agency_id');
      return;
    }
  ...
  const fetchReturns = async () => {
    if (!user || !profile?.agency_id) {
      console.log('Cannot fetch GST returns: user not authenticated or no agency_id');
      return;
    }
