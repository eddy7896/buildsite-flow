...
async function fixAgencyDatabase(dbName) {
  ...
  try {
    ...
    if (error.code === '3D000') {
      console.log(`[${dbName}] ⚠️  Database does not exist, skipping...`);
      return false;
    }
    throw error;
  }
  ...
  try {
    ...
    // Create leave_requests table if it doesn't exist, then fix columns
    await client.query(`
    ...
    // Fix leave_requests table - add employee_id and agency_id if missing
    await client.query(`
      DO $$ 
      BEGIN
        ...
        IF NOT EXISTS (
          ...
          UPDATE public.leave_requests SET employee_id = user_id WHERE employee_id IS NULL AND user_id IS NOT NULL;
    ...
    // Create missing tables if they don't exist (invoices and reimbursement_requests already created above if needed)
    // Note: leave_requests was created above
