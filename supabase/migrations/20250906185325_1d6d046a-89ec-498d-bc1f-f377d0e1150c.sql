-- Create modern GST tables with correct foreign key reference
...
-- Create auto-set agency trigger
CREATE TRIGGER set_gst_settings_agency_id
  BEFORE INSERT ON public.gst_settings
  FOR EACH ROW EXECUTE FUNCTION public.set_agency_id();

CREATE TRIGGER set_gst_returns_agency_id
  BEFORE INSERT ON public.gst_returns
  FOR EACH ROW EXECUTE FUNCTION public.set_agency_id();

CREATE TRIGGER set_gst_transactions_agency_id
  BEFORE INSERT ON public.gst_transactions
  FOR EACH ROW EXECUTE FUNCTION public.set_agency_id();
