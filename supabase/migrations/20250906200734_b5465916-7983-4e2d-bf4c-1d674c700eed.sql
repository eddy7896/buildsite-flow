-- Phase 1: Immediate Security Fixes for Role-Based Access Control
...
-- 4. Update payments and financial management access
UPDATE TABLE public.invoices;

-- 5. Add department-based access control for employee data
...
-- 6. Update employee details policy with department restrictions
DROP POLICY IF EXISTS "Only HR can view employee details" ON public.employee_details;

CREATE POLICY "Department-based employee data access"
ON public.employee_details
FOR SELECT
USING (public.can_access_employee_data(user_id));

CREATE POLICY "HR and authorized roles can manage employee details"
ON public.employee_details
...
-- 7. Update user roles policy to prevent unauthorized role changes
...
-- 8. Create audit logging trigger for sensitive operations
