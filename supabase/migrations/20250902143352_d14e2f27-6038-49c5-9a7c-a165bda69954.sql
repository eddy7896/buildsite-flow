-- Phase 2C: Enhanced Leave Management
...
BEGIN
  ...
  IF TG_OP = 'UPDATE' THEN
    ...
    -- If request was changed back to pending (resubmitted)
    ELSIF OLD.status IN ('approved', 'rejected') AND NEW.status = 'pending' THEN
      -- Add to pending, remove from used if it was approved
...
BEGIN
  ...
  PERFORM public.create_notification(
    ...
    'Your leave balances for ' || target_year || ' have been set up.',
    jsonb_build_object('year', target_year),
    'normal',
    '/my-leave'
  );
END;
